...
try {
    ...
    $hasAssignee = ((int)$assigneeStmt->get_result()->fetch_assoc()['total']) > 0;
    ...
    if ($projectIds) {
        ...
        $govStmt = $conn->prepare("
            SELECT project_id, item_key, item_label, stage_gate, status
            FROM governance_items
            WHERE project_id IN ($placeholders)
            ORDER BY FIELD(stage_gate, 'initiation','planning','execution','qa','uat','closure'), id
        ");
