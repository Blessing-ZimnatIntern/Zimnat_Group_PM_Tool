...
$sql = "
    SELECT p.id, p.name, p.stage, p.status, p.owner, p.progress, p.gate_due,
    ...
    <table class="data-table">
        ...
        <tbody>
            <?php if (empty($projects)): ?>
                <tr><td colspan="8">No projects match the filters.</td></tr>
            <?php else: foreach ($projects as $p): ?>
                <tr>
                    ...
                    <td><?= $p['progress'] ?>%</td>
                    <td style="<?= ($p['is_gate_overdue'] == 1) ? 'color:#dc2626;font-weight:bold;' : '' ?>">
                        <?= htmlspecialchars($p['gate_due'] ?? '—') ?>
                        <?= ($p['is_gate_overdue'] == 1) ? '⚠️' : '' ?>
                    </td>
                </tr>
